<template>
<div id="app" class="flex container m-auto w-full h-screen ">
    <!--** side nav bar **-->
    <div class="border-r border-lighter px-2 lg:px-6 py-2 max-w-min md:w-1/5 flex flex-col justify-between">
        <!-- menu buttons -->
        <div>
            <button class="h-12 w-12 rounded-full text-blue hover:bg-lightblue text-3xl ">
                <i class="fa-brands fa-twitter"></i>
            </button>
            <div>
                <button v-for="(tab, index) in tabs" :key="index" @click="id = tab.id" :class="`flex focus:outline-none mb-2 hover:text-blue items-center py-2 px-4 hover:bg-lightblue rounded-full mr-auto ${id===tab.id? 'text-blue' : ''}`">
                    <i class="text-2xl mr-4 text-left" :class="tab.icon"></i>
                    <p class="text-lg font-semibold text-left hidden lg:block">{{tab.title}}</p>
                </button>
            </div>
            <button class="text-white bg-blue mb-2 rounded-full w-12 h-12 lg:h-auto lg:w-full p-3 font-semibold hover:bg-darkblue focus:outline-none">
        ...
        <div class="lg:w-full relative">
            <button @click="dropdown=!dropdown" class="flex items-center rounded-full w-full hover:bg-lightest p-2 focus:outline-none">
                <img src="../assets/profile.png" alt="profile-img" class="w-10 h-10 rounded-full border border-lighter">
                <div class="hidden lg:block ml-4">
                    <p class="text-sm font-bold leading-tight">Himanshu</p>
                    <p class="text-sm leading-tight">@hms@kr</p>
                </div>
                <i class="hidden lg:block fas fa-angle-down ml-auto text-lg"></i>
            </button>
            <!-- dropdown -->
            <div v-if="dropdown===true" class="absolute bottom-0 left-0 w-64 lg:w-full rounded-lg shadow-lg border-lightest bg-white mb-16">
                <button @click="dropdown=false" class="p-3 flex items-center w-full hover:bg-lightblue focus:outline-none">
                    <img src="../assets/profile.png" alt="profile-img" class="w-10 h-10 rounded-full border border-lighter">
                    <div class="ml-4">
                        <p class="text-sm font-bold leading-tight">Himanshu Kr</p>
                        <p class="text-sm leading-tight">@hms@kr</p>
                    </div>
    ...
    <!-- Tweets Section 19:46 -->
    <div class="w-full md:w-1/2 h-full scrollbar-hide overflow-y-scroll">
        ...
        <div v-for="(tweet, index) in tweets" :key="index" class="w-full p-4 border-b hover:bg-lighter flex">
          ...
          <div class="w-full">
            <div class="flex items-center w-full">
              <p class="font-semibold"> Himanshu Kr </p>
    ...
    <!-- trending -->
    <div class="md:block hidden w-1/3 h-full border-l border-lighter py-2 px-6 scrollbar-hideoverflow-y-scroll relative">
      ...
      <div class="w-full rounded-lg bg-lightest my-4">
        ...
        <button v-for="(friend, index) in friends" :key="index" class="w-full flex hover:bg-lighter p-3 border-t border-lighter">
          ...
          <!-- <img :src="require(`../assets/${friend.src}`)" class="w-12 h-12 rounded-full border border-lighter" /> -->
          <div class="hidden lg:block ml-4">
